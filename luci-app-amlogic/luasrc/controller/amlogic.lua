...
function index()
	...
	local platfrom = luci.sys.exec("cat /etc/lasthinker-openwrt-release 2>/dev/null | grep PLATFORM | awk -F'=' '{print $2}' | grep -oE '(amlogic|rockchip|allwinner|qemu)'") or "Unknown"
...
device_platfrom = trim(luci.sys.exec("cat /etc/lasthinker-openwrt-release 2>/dev/null | grep PLATFORM | awk -F'=' '{print $2}' | grep -oE '(amlogic|rockchip|allwinner|qemu)'")) or "Unknown"
