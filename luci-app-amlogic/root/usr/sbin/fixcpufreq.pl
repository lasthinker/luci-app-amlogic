...
sub fix_config_file {
	...
	# If the governor is not set, or does not exist, the default value for repair failure is schedutil
	...
	# If an illegal minfreq appears, modify it to the actual min_freq
	my $config_min_freq = &uci_get_by_type($config_name, "settings", "minfreq" . ${id}, "0");
	if($freq_hash{$config_min_freq} != 1) {
		&uci_set_by_type($config_name, "settings", "minfreq" . ${id}, $min_freq);
	}

	# If there is an illegal maxfreq
	# Or maxfreq < minfreq, then modify it to the actual max_freq
...
sub uci_get_by_type {
	...
	# Eliminate carriage return and line feed
	$ret =~ s/[\n\r]//g;
	if($ret eq '') {
		return $default;
	} else {
