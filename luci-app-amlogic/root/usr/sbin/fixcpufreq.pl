...
sub fix_invalid_value {
	...
	# If the governor is not set, or does not exist, the repair failure default value is schedutil
	...
	# If there is a minfreq that does not exist, modify it to the actual min_freq
	if($uci_config eq "cpufreq" ) {
	...
	# If there is a maxfreq that does not exist
