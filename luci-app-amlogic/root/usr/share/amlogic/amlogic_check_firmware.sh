...
support_platform=("amlogic" "qemu-aarch64")
...
check_updated() {
    ...
    api_lasthinker_down_line="$(echo ${api_updated_merge[*]} | tr ' ' '\n' | grep ${latest_updated_at} | cut -d '@' -f1)"
    ...
    # Prompt to update
    if [[ -n "${api_lasthinker_down_line}" && -n "$(echo ${api_lasthinker_down_line} | sed -n "/^[0-9]\+$/p")" ]]; then
        tolog '<input type="button" class="cbi-button cbi-button-reload" value="Download" onclick="return b_check_firmware(this, '"'download_${api_lasthinker_down_line}_${latest_updated_at}'"')"/> Latest updated: '${latest_updated_at_format}'' "1"
    else
